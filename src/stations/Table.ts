...
export class Table {
  ...
  constructor(x: number, z: number, scene: THREE.Scene, flyer: Flyer) {
    const g = this.group;
    ...
    for (const side of [-1, 1]) {
      const cx = side * 0.95;
      g.add(at(box(0.5, 0.08, 0.5, C.primary), cx, 0.45, 0));
      g.add(at(box(0.36, 0.41, 0.36, C.primaryDark), cx, 0.2, 0));
      g.add(at(box(0.08, 0.55, 0.5, C.primary), cx + side * 0.22, 0.75, 0));
      ...
      this.seats.push({
        ...
        occupant: null,
        plate: new ItemStack(plateAnchor, flyer, () => 10, columnLayout),
        table: this,
      });
    }
